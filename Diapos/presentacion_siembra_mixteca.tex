...
% Configurar fondo con imagen semi-transparente hacia negro para TODAS las diapositivas
\usebackgroundtemplate{%
\begin{tikzpicture}[remember picture,overlay]
  % Primero el fondo negro base
  \fill[black] (current page.south west) rectangle (current page.north east);
  % Luego la imagen con opacidad
  \node[opacity=0.35, inner sep=0pt] at (current page.center) {
     \includegraphics[width=\paperwidth,height=\paperheight,keepaspectratio=false]{fondo_maiz.png}
  };
\end{tikzpicture}
}
...
% =================================================================================
% SECCION: OPTIMIZACION PSO (Nuevo)
% =================================================================================

% INTRO PSO
\begin{frame}{Alternativa: Enjambre de Particulas (PSO)}
  \justifying
  \textbf{Que es PSO?}
  \begin{itemize}
    \item \textbf{Particle Swarm Optimization}: Algoritmo inspirado en el comportamiento de bandadas de aves o cardumenes.
    \item Cada ``particula'' representa una solucion candidata (un dia de siembra).
    \item Las particulas se mueven por el espacio de busqueda, influenciadas por su mejor posicion historica y la mejor del grupo.
  \end{itemize}
  
  \vspace{0.5em}
  \textbf{Ventajas vs Algoritmo Genetico:}
  \begin{itemize}
    \item Menos parametros de configuracion.
    \item Convergencia mas suave y predecible.
    \item No requiere operadores de cruce ni mutacion.
  \end{itemize}
\end{frame}

% CODIGO PSO
\begin{frame}[fragile]{Codigo: Optimizacion PSO (Mealpy)}
  \scriptsize
  \begin{verbatim}
...
% =================================================================================
% SECCION: PIPELINE DE DATOS (Nuevo)
% =================================================================================

\begin{frame}{Pipeline Completo de Datos}
  \centering
  \textbf{Flujo de Archivos del Sistema}
  
  \vspace{0.5em}
  \footnotesize
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \textcolor{myaccent}{\textbf{Scripts de Procesamiento:}}
      \begin{enumerate}
        \item \texttt{preparacion\_datos.py}\\
        {\tiny Descarga datos NASA POWER, limpia y prepara variables ciclicas.}
        \item \texttt{entrenamiento\_modelo.py}\\
        {\tiny Diseña y entrena la LSTM con Keras.}
        \item \texttt{generar\_pronostico.py}\\
        {\tiny Usa el modelo para predecir 2026.}
      \end{enumerate}
    \end{column}
    \begin{column}{0.48\textwidth}
      \textcolor{myaccent}{\textbf{Archivos Generados:}}
      \begin{itemize}
        \item \texttt{Reporte\_Humano\_Huajuapan.csv}\\
        {\tiny Datos historicos limpios para validacion.}
        \item \texttt{Dataset\_Entrenamiento\_IA.csv}\\
        {\tiny Ventanas de 15 dias para entrenar LSTM.}
        \item \texttt{mejor\_modelo\_clima.h5}\\
        {\tiny Red neuronal entrenada (pesos).}
        \item \texttt{Pronostico\_2026\_IA.csv}\\
        {\tiny Prediccion diaria para el AG/PSO.}
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}

\begin{frame}{Detalle: Preparacion de Datos (NASA POWER)}
  \justifying
  \textbf{Fuente de Datos:}
  \begin{itemize}
    \item API de \textbf{NASA POWER}: Datos satelitales de temperatura y precipitacion.
    \item Ubicacion: Huajuapan de Leon, Oaxaca.
    \item Periodo: 19 anos de historia (hasta 2024).
  \end{itemize}
  
  \vspace{0.5em}
  \textbf{Procesamiento Aplicado:}
  \begin{enumerate}
    \item \textbf{Limpieza}: Eliminacion de valores nulos y anomalos.
    \item \textbf{Codificacion Ciclica}: Transformacion de fechas a Seno/Coseno para capturar estacionalidad.
    \item \textbf{Normalizacion}: Escalado de variables para la red neuronal.
    \item \textbf{Ventaneo}: Creacion de secuencias de 15 dias para entrada LSTM.
  \end{enumerate}
\end{frame}
